        ...
        <tbody>
            @foreach ($students as  $key=> $student)
                <tr>
                    ...
                    <td>{{ $student->phone }}</td>
                    <td>{{ $student->email }}</td>
                    <td>  <img width="100"  src="{{ $student->photo }}" alt="{{$student->name}}" srcset=""> </td>
                    <td class="btn-group">
                        <a class="btn btn-info" href="{{ route('students.show', $student->id) }}">Show</a>
                        <a class="btn btn-secondary" href="{{ route('student.restore', $student->id) }}">Restore</a>
                        <a class="btn btn-warning" href="{{ route('student.delete', $student->id) }}">Delete</a>
                    </td>
                </tr>
            @endforeach
        </tbody>
