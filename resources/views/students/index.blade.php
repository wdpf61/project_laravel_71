...
@section('content')
    ...
    <form class="input-group mb-3" action="{{ route('students.index') }}" method="get">
        <input value="{{ old('search', $search) }}" id="select-field" type="text" name="search" class="form-control"
        ...
        <tbody>
            @foreach ($students as $key => $student)
                <tr>
                    ...
                    <td>{{ $student->phone }}</td>
                    <td>{{ $student->email }}</td>
                    <td> <img width="100" src="{{ asset('') }}/uploads/{{ $student->photo }}"
                            alt="{{ $student->name }}" srcset=""> </td>
                    <td class="btn-group">
                        @can('view', $student)
